...
function getTips(sql, explanation) {
    ...
    Object.keys(userindexMatches).forEach(table => {
        if (new RegExp(`\\b${table}\\b[^]*\\b${userindexMatches[table]}`, 'i').test(sql)) {
            pushTip('0', `You appear to be querying the <code>${table}</code> table and filtering by user. ` +
    ...
    specialViews.forEach(table => {
        ...
        if (matches) {
            pushTip('0', `You appear to be querying <code>${matches[1]}</code> and <code>${table}</code>. ` +
    ...
    explanation.forEach((plan, index) => {
        ...
        if (plan.rows > 1000000) {
            pushTip(index, `Query plan ${plan.id}.${index + 1} scans over a million rows. Your query could likely be improved, ` +
                `or broken out into multiple queries to improve performance.`);
        }
    });

    if (Object.keys(tips).length) {
        pushTip('*', 'When running potentially slow queries in your application, consider prepending ' +
    ...
    if (Object.keys(tips).length && /revision(?:_userindex)?|logging(?:_logindex)?/i.test(sql)) {
        pushTip('*', 'If you only need to query for recent revisions and log actions (within the last 30 days), ' +
