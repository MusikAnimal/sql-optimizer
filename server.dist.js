...
const env = JSON.parse(fs.readFileSync('env.json', 'utf8')); // Create a server with a host and port
...
function validate(sql) {
  if (/SET.*?max_statement_time\s*=\s*/i.test(sql)) {
    return {
      error: 'Query error: max_statement_time cannot be set when using this tool.'
    };
  }
...
function injectSleep(sql) {
  ...
  sql = sql.replace(/\bSELECT\s/gi, `SELECT SLEEP(${TIMEOUT}), `); // Remove any SLEEPs that were added to a subquery that used with an IN clause.
